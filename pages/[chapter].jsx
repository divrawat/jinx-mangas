...
export default function Chapter({ chapterNumber, imageUrls, totalChapters, params, errorcode }) {
    ...
    const schema = {
        ...
        "@graph": [
            ...
            },
            {
                ...
                "thumbnailUrl": `${IMAGES_SUBDOMAIN}/chapter-${chapterNumber}/1.webp`,
                "description": `Read ${MANGA_NAME} ${MANGA_TYPE} Online at ${DOMAIN_NAME} in HD quality.`,
                "breadcrumb": {
    ...
    const encodedTitle = `${MANGA_NAME} ${MANGA_TYPE} Chapter ${chapterNumber}`;
    ...
    return (
        <>
            ...
            <Navbar />
            <article>
                <h1 className="text-3xl font-bold text-center text-[white] px-5 pt-5 md:my-5">{`${MANGA_NAME} ${MANGA_TYPE} Chapter ${chapterNumber}`}</h1>

                <div className='flex justify-center px-5 text-[#85e1e6] text-[13px] mb-5'>
                    <div><a href={DOMAIN}>Home</a></div>
                    <div className='px-2'>{` -> `}</div>
                    <div><a href={`${DOMAIN}/chapter-${chapterNumber}`}>{`${MANGA_NAME} ${MANGA_TYPE} Chapter ${chapterNumber}`}</a></div>
                </div>
